...
#define DECIMATION_VAL 4 //HIGH FREQ RESOLUTION = 8 (>3k), LOW = 4
#define DECIMATED_CLOCK CLOCK/DECIMATION_VAL
#define DECIMATED_SAMPLES SAMPLES/DECIMATION_VAL
#define DECIMATED_M M-2 //HIGH FREQ RESOLUTION = M-, LOW = M-
...
int countQBuffer = 0;
int countFindNote = 0;
int countStream = 0;	// start + wait, not read
...
void timer0Handler() {
	...
	if (a <= 0x2a30 && a >= 0x2144)
	{
		countFFT++;
	}

	else if(a <= 0x2ba4 && a >= 0x2a94 )
	{
		countQBuffer++;
	}
	else if(a <= 0x3384 && a >= 0x3194)
	{
		countFindNote++;
	}
	else if((a <= 0x3514 && a >= 0x34d4) || (a <= 0x34d0 && a >= 0x34a4))
	{
		countStream++;
	}
...
//		xil_printf("countFindNote = %i\r\n", countFindNote);
//		xil_printf("countStream = %i\r\n", countStream);
		countFFT =  countQBuffer = countFindNote = countStream = 0;
	}
