...
char notes[12][1] = { "C", "Cs", "D", "Ds", "E", "F", "Fs", "G", "Gs", "A",
		"Bb", "B" };
		...
		for (int i = 0; i < 11; i++) {
			...
			if (frequency >= note_low_end && frequency <= note_high_end) { //frequency is in range, check what its closer to.
				...
				if (*cent < 50) { //closer to lower end note
					...
					*note = i + 1; //closer to higher note
					*cent = (*cent) - 100;
				}
				break;
			}
...
void bars() {
	...
	} else if (previousCent >= 21 && previousCent <= 30) {
		setColor(0, 0, 0);
		if (cent <= 20) {
			// +30
	...
	} else if (previousCent >= 31 && previousCent <= 40) {
		setColor(0, 0, 0);
		if (cent <= 30) {
			// +40
	...
	} else if (previousCent >= 41 && previousCent <= 50) {
		setColor(0, 0, 0);
		if (cent <= 40) {
			// +50
...
//xil_printf("octaveSelection\n");
	...
	case FREQUENCY: {
		...
		//xil_printf("note = %i\r\n", note);
		if (previousCent == cent && cent != -1000) {

		} else if (previousCent != cent && cent != -1000) {
